...
function loadState() {
    try {
        ...
        // Restaurar canciones — eliminar demos de prueba siempre
        const DEMO_TITLES = ['Summer Vibes', 'Midnight Dreams', 'Urban Flow', 'Neon Lights', 'Ocean Breeze', 'Velvet Thunder', 'Golden Hour', 'Deep Space'];
        if (saved.songs) {
            const filtered = saved.songs.filter(s => !DEMO_TITLES.includes(s.title));
            appState.songs = filtered.map(s => ({ ...s, file: null }));
            if (filtered.length !== saved.songs.length) {
                const updatedSave = { ...saved, songs: filtered };
                localStorage.setItem('musicflow_state', JSON.stringify(updatedSave));
            }
        }
        return true;
